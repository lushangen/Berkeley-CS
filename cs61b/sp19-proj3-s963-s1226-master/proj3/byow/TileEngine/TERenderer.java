...
public class TERenderer {
    ...
    public void renderWithText(TETile[][] world, double mX, double mY) {
        int numXTiles = world.length;
        int numYTiles = world[0].length;
        ...
        Date date = new Date();
        StdDraw.text(60, 32.5, formatter.format(date));
        if (!(mX < 0 || mX >= width || mY < 0 || mY >= height - 5)) {
            String newStr = world[(int) mX][(int) mY].description();
            if (!newStr.equals("Nothing")) {
                StdDraw.setPenColor(Color.WHITE);
                font = new Font("Monaco", Font.BOLD, 30);
                StdDraw.setFont(font);
                StdDraw.text(7.5, 32.5, newStr);
            }
        }
