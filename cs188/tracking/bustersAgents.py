...
class GreedyBustersAgent(BustersAgent):
    ...
    def chooseAction(self, gameState):
        ...
        ghostDist = [dist.argMax() for dist in livingGhostPositionDistributions]

        for ghost in ghostDist:
            curr = self.distancer.getDistance(ghost, pacmanPosition)
            if minDist > curr:
                minDist = curr
                closestGhost = ghost

        for action in legal:
            curr = self.distancer.getDistance(Actions.getSuccessor(pacmanPosition, action), closestGhost)
            if minVal > curr:
                minVal = curr
                greedyAction = action
