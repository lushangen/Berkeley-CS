...
class QLearningAgent(ReinforcementAgent):
    ...
    def computeActionFromQValues(self, state):
        ...
        if not self.getLegalActions(state):
            return None
        return random.choice([action for action in self.getLegalActions(state) if self.getValue(state) ==
...
class ApproximateQAgent(PacmanQAgent):
    ...
    def update(self, state, action, nextState, reward):
        ...
        for i in self.featExtractor.getFeatures(state, action):
            self.getWeights()[i] += self.alpha * difference * self.featExtractor.getFeatures(state, action)[i]
